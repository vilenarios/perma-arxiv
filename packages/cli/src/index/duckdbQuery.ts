...
export class DuckDBQuery {
  ...
  async aggregateByTimeRange(
    ...
    let sql = `
      SELECT
        strftime('${dateFormat}', published) as time_bucket,
        COUNT(*) as count
      FROM papers
      WHERE 1=1
    `;
    ...
    result.forEach(row => {
      aggregated.set(row.time_bucket, row.count);
    });

    return aggregated;
  }

  async findSimilarPapers(paperId: string, limit: number = 10): Promise<ParquetPaperRecord[]> {
    await this.initialize();

    // First get the paper
    const paperResult = await new Promise<any>((resolve, reject) => {
      this.conn.all('SELECT * FROM papers WHERE id = ? LIMIT 1', paperId, (err, rows: any[]) => {
        if (err) reject(err);
    ...
    const categories = paperResult.all_categories.split(',').map((c: string) => c.trim());

    const sql = `
      SELECT *
      FROM papers
      WHERE id != ?
        AND (${categories.map(() => 'all_categories LIKE ?').join(' OR ')})
      ORDER BY published DESC
      LIMIT ?
    `;
