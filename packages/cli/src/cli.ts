...
program
  ...
  .option('-d, --delay <number>', 'Delay between batches in ms', parseInt, 1000)
...
program
  .command('upload:cost')
  .description('Estimate upload costs for all unuploaded papers or a specific size')
  .option('-s, --size-mb <number>', 'File size in MB to estimate (if not specified, calculates for all unuploaded papers)', parseFloat)
  .option('-w, --wallet <path>', 'Path to Arweave wallet JWK file', process.env.ARWEAVE_WALLET_PATH || './wallet.json')
  .action(async (options) => {
    ...
    try {
      ...
      if (options.sizeMb) {
        ...
        // Calculate total cost for all unuploaded papers
        console.log('\n📊 Calculating upload costs for all unuploaded papers...\n');

        const papers = await db.getReadyForUpload(1000000); // Get all ready for upload

        if (papers.length === 0) {
          console.log('No papers ready for upload!');
          await db.close();
          return;
        }

        console.log(`Found ${papers.length} papers ready to upload`);

        // Calculate total size by checking actual files
        let totalSize = 0;
        let fileCount = 0;
        ...
        console.log(`\nTotal papers: ${fileCount}`);
        console.log(`Total size: ${totalSizeGB} GB (${totalSizeMB} MB)`);

        // Get upload cost
        const cost = await uploader.getUploadCost(totalSize);
        ...
        const hasEnough = BigInt(balance.winc) >= BigInt(cost.winc);

        if (hasEnough) {
          console.log(`\n✅ You have enough balance to upload all papers!`);
          const remaining = BigInt(balance.winc) - BigInt(cost.winc);
          const remainingAR = (Number(remaining) / 1e12).toFixed(6);
          console.log(`   Remaining after upload: ${remaining.toString()} winc (${remainingAR} AR)`);
        } else {
          console.log(`\n❌ Insufficient balance!`);
          const needed = BigInt(cost.winc) - BigInt(balance.winc);
          const neededAR = (Number(needed) / 1e12).toFixed(6);
          console.log(`   Additional needed: ${needed.toString()} winc (${neededAR} AR)`);
        }
...
program
  .command('web:deploy')
  .description('Deploy web app folder to Arweave and update arxiv ArNS')
  .argument('<dist-folder>', 'Path to dist folder (e.g., packages/web/dist)')
  .option('-w, --wallet <path>', 'Path to Arweave wallet', process.env.ARWEAVE_WALLET_PATH || './wallet.json')
  .option('--index <file>', 'Index file name', 'index.html')
  .action(async (distFolder: string, options) => {
    try {
      const { TurboUploader } = await import('./arweave/turboUploader');
      const { ArnsManager } = await import('./arweave/arnsManager');
      ...
      console.log('\n🚀 Deploying web app to Arweave...\n');

      // Verify dist folder exists
      const folderPath = path.resolve(distFolder);
      try {
      ...
      // Upload entire folder with manifest
      logger.info('Uploading web app folder', { folder: folderPath });
      console.log(`📁 Uploading folder: ${folderPath}`);
      console.log(`📄 Index file: ${options.index}\n`);

      const uploadResult = await uploader.uploadFolder(folderPath, {
        indexFile: options.index,
        tags: [
          { name: 'App-Name', value: 'permanent-arxiv' },
          { name: 'App-Version', value: '2.0' },
          { name: 'Content-Type', value: 'application/x.arweave-manifest+json' },
        ],
      });

      if (!uploadResult || !uploadResult.manifestId) {
        throw new Error('Upload failed - no manifest ID returned');
      }

      console.log(`✅ Web app uploaded successfully!`);
      console.log(`   Manifest ID: ${uploadResult.manifestId}`);
      console.log(`   Files uploaded: ${uploadResult.fileResponses.length}`);
      console.log(`   https://arweave.net/${uploadResult.manifestId}\n`);

      // Update ArNS
      console.log('🔄 Updating arxiv ArNS record...\n');
      const arnsManager = new ArnsManager(options.wallet);
      const result = await arnsManager.updateMainSite(uploadResult.manifestId);
